...
export default {
	methods: {
		 ...
		 * @throws {Error}
		 */
		async createShare({ path, shareType, shareWith, publicUpload, password, permissions }) {
			try {
				const request = await axios.post(shareUrl, { path, shareType, shareWith, publicUpload, password, permissions, format })
